// Fintech promo composition — a React/TS port of `Fintech Design/promo/scenes.jsx`.
// A 30-second, 1080×1080 promo with six scenes. Language ('en' | 'ar') selects
// localized copy, font stack and text direction.

import type { CSSProperties, ReactNode } from 'react';
import { Easing as E, clamp, Sprite, useSprite, useTime } from './engine';
...
function LogoMark({ size = 56, radius }: { size?: number; radius?: number }) {
...
function Eyebrow({ text, lang }: { text: string; lang: Lang }) {
  const isAr = lang === 'ar';
  return (
    <span
      style={{
...
function SceneDots({ total = 6, active = 0 }: { total?: number; active?: number }) {
  return (
    <div
      style={{
        position: 'absolute',
...
function Legend({ color, label, lang }: { color: string; label: string; lang: Lang }) {
  return (
    <div
      style={{
        display: 'flex',
...
// ── SCENE 1 — Hook (0–4s) ────────────────────────────────────────────────────
function SceneHook({ lang }: { lang: Lang }) {
  const c = COPY[lang];
  const { localTime, duration } = useSprite();
  ...
  const blots = [
    ...
    { x: 90, y: 480, amt: '−$89', delay: 0.2 },
    { x: 900, y: 540, amt: '−$210', delay: 0.45 },
    { x: 540, y: 180, amt: '−$12', delay: 0.7 },
    { x: 540, y: 900, amt: '−$76', delay: 0.8 },
  ];
...
function SceneBrand({ lang }: { lang: Lang }) {
  const c = COPY[lang];
  const { localTime, duration } = useSprite();
    ...
    <>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 28 }}>
          <div style={{ transform: `scale(${markT})`, transformOrigin: 'center' }}>
            <LogoMark size={180} />
          </div>
          <div
            style={{
...
function SceneQuickAdd({ lang }: { lang: Lang }) {
  const c = COPY[lang];
  const { localTime, duration } = useSprite();
  const isAr = lang === 'ar';
  ...
  const amtVal = (54.2 * E.easeOutCubic(amtProg)).toFixed(2);
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 20, marginBottom: 28 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              >
                <span style={{ color: T.text2, marginRight: 4 }}>$</span>
                {amtVal}
              </div>
            </div>
          </div>
        ...
        {toastVisible && (
          ...
          >
            ...
            <div style={{ fontFamily: c.fontStack, fontSize: 22, fontWeight: 600, color: T.text }}>
              {c.s3_toast}
            </div>
          </div>
        )}
...
function SceneAnalytics({ lang }: { lang: Lang }) {
  const c = COPY[lang];
  const { localTime, duration } = useSprite();
  const isAr = lang === 'ar';
        ...
        >
          ...
          >
            ...
            >
              ...
              <div style={{ fontFamily: c.fontStack, fontSize: 16, fontWeight: 500, color: T.text2 }}>
                {c.s4_delta_sub}
              </div>
            </div>
          </div>
          ...
          >
            {data.map((d, i) => {
              ...
              return (
                ...
                >
                  <div style={{ flex: 1, height: iH, background: T.lime, borderRadius: '8px 8px 0 0' }} />
                  <div
                    style={{
                      flex: 1,
                      height: eH,
...
function SceneFree({ lang }: { lang: Lang }) {
  const c = COPY[lang];
  const { localTime, duration } = useSprite();
  const isAr = lang === 'ar';
  ...
  return (
    <>
      ...
      >
        <div style={{ opacity: eyebrowT, transform: `scale(${0.6 + 0.4 * eyebrowT})` }}>
...
function SceneCTA({ lang }: { lang: Lang }) {
  const c = COPY[lang];
...
// ── Master composition ───────────────────────────────────────────────────────
export function FintechPromo({ lang = 'en' }: { lang?: Lang }) {
  const time = useTime();
  const activeIdx =
    time < 4 ? 0 : time < 8 ? 1 : time < 15 ? 2 : time < 22 ? 3 : time < 27.5 ? 4 : 5;
    ...
    >
      <div
        style={{
          ...
          inset: 0,
          background:
            'radial-gradient(ellipse 80% 60% at 50% -20%, rgba(212, 240, 60, 0.05) 0%, transparent 60%)',
          pointerEvents: 'none',
        }}
      />
      ...
      <Sprite start={4} end={8.2}>
        <SceneBrand lang={lang} />
      </Sprite>
      <Sprite start={8} end={15.2}>
        <SceneQuickAdd lang={lang} />
      </Sprite>
      <Sprite start={15} end={22.2}>
        <SceneAnalytics lang={lang} />
      </Sprite>
      <Sprite start={22} end={27.7}>
        <SceneFree lang={lang} />
      </Sprite>
      <Sprite start={27.5} end={30}>
        <SceneCTA lang={lang} />
      </Sprite>
